...
export function ProjectDevelopmentWizard({
  ...
  const autoGeneratedTitleLoglineRef = useRef(new Set<string>());
  ...
  useEffect(() => {
    const shouldAutofill =
      activeStep === "title-logline" &&
      data.development?.startMode === "import" &&
      conceptValue.trim().length > 0 &&
      (!data.title.trim() || !data.logline.trim()) &&
    ...
    const autofillKey = [
      data.development?.sourceTextKey || "no-source",
      conceptValue.trim(),
      data.title.trim(),
      data.logline.trim(),
    ].join("::");

    if (autoGeneratedTitleLoglineRef.current.has(autofillKey)) {
      return;
    }

    autoGeneratedTitleLoglineRef.current.add(autofillKey);
    void generateTitle();
  }, [
    activeStep,
    conceptValue,
    data.development?.sourceTextKey,
    data.development?.startMode,
    data.logline,
    data.title,
    loadingStep,
    generateTitle,
      ...
      case "title-logline":
        return (
          <div className="space-y-6">
            ...
            >
              <Bot className="mr-2 h-4 w-4" />
              {loadingStep === "title-logline" ? "Generating..." : "Regenerate Title + Logline"}
            </Button>
          </div>
        );
